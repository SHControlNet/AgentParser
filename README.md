<img src="https://p82.cooltext.com/Rendered/Cool%20Text%20-%20AgentParser%20446281810181357.png" width="400">
...
You get a nice object which has information about the browser, rendering engine, os and device. It gives you names and versions and even device manufacturer and model. And AgentParser is pretty tenacious. It gives you info that others don't. For example:
...
AgentParser requires with PHP 7.0 or higher and supports PHP 8. AgentParser is compatible with the PSR-4 autoloading standard and follows PSR-1 and PSR-2 coding style.


How to install it
-----------------

You can install AgentParser by using Composer - the standard package manager for PHP. The package is called `AgentParser/parser`.

    composer require AgentParser/parser

You can easily update AgentParser by running a simple command.

    composer update AgentParser/parser
...
The second step is to create a new `AgentParser\Parser` object. This object will contain all the information the library could find about the browser. The object has a required parameter, either the headers send by the browser, or a useragent string. Using the headers is preferable, because it will allow a better detection, but if you have just the useragent string, this will also work.

For example:

```php
$result = new AgentParser\Parser(getallheaders());
```

or:

```php
$result = new AgentParser\Parser($_SERVER['HTTP_USER_AGENT']);
```
...
In some cases you may want to disable the detection of bots. This allows the bot the deliberately fool AgentParser, so you can pick up the identity of useragent what the bot tries to mimic. This is especially handy when you want to use AgentParser to switch between different variants of your website and want to make sure crawlers see the right variant of the website. For example, a bot that mimics a mobile device will see the mobile variant of you site.

```php
$result = new AgentParser\Parser(getallheaders(), [ 'detectBots' => false ]);
```

Enable result caching
---------------------

AgentParser supports PSR-6 compatible cache adapters for caching results between requests. Using a cache is especially useful if you use AgentParser on every page of your website and a user visits multiple pages. During the first visit the headers will be parsed and the result will be cached. Upon further visits, the cached results will be used, which is much faster than having to parse the headers again and again.

There are adapters available for other types of caches, such as APC, Doctrine, Memcached, MongoDB, Redis and many more. The configuration of these adapters all differ from each other, but once configured, all you have to do is pass it as an option when creating the `Parser` object, or use the `setCache()` function to set it afterwards. AgentParser has been tested to work with the adapters provided by [PHP Cache](http://php-cache.readthedocs.org/en/latest/). For a list of other packages that provide adapters see [Packagist](https://packagist.org/providers/psr/cache-implementation).
...
And change the call to AgentParser/Parser as follows:
...
$result = new AgentParser\Parser(getallheaders(), [ 'cache' => $pool ]);
```
...
After a new `AgentParser\Parser` object is created, it contains a number of properties and functions. All of these properties are guaranteed to be present.
...
An object of the `AgentParser\Model\Browser` class is used for the `browser` property of the main `AgentParser\Parser` object and contains a number of properties. If a property is not applicable in this situation it will be null or undefined.
...
An object of the `AgentParser\Model\Engine` class is used for the `engine` property of the main `AgentParser\Parser` object and contains a number of properties. If a property is not applicable in this situation it will be null or undefined.
...
An object of the `AgentParser\Model\Os` class is used for the `os` property of the main `AgentParser\Parser` object and contains a number of properties. If a property is not applicable in this situation it will be null or undefined.
...
An object of the `AgentParser\Model\Device` class is used for the `device` property of the main `AgentParser\Parser` object and contains a number of properties. If a property is not applicable in this situation it will be null or undefined.
...
An object of the `AgentParser\Model\Family` class is used for the `family` property of the `AgentParser\Model\Browser` and `AgentParser\Model\Os` object and contains a number of properties. If a property is not applicable in this situation it will be null or undefined.
...
An object of the `AgentParser\Model\Using` class is used for the `using` property of the `AgentParser\Model\Browser` object and contains a number of properties. If a property is not applicable in this situation it will be null or undefined.
...
An object of the `AgentParser\Model\Version` class is used for the `version` property of the `browser`, `engine` and `os` object and contains a number of properties and functions. If a property is not applicable in this situation it will be null or undefined.
